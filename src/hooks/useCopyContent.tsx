...
export default function useCopyContent(options: UseCopyContentOptions = {}) {
  ...
  const copyFromElement = useCallback((element: HTMLElement) => {
    const textContent = element.innerText || element.textContent || ''
    copy(textContent)
    
    const description = textContent.length > maxDescriptionLength 
      ? textContent.substring(0, maxDescriptionLength) + '...' 
      : textContent
  ...
  const copyRawContent = useCallback((content: string) => {
    copy(content)
    
    const description = content.length > maxDescriptionLength 
      ? content.substring(0, maxDescriptionLength) + '...' 
      : content
  ...
  const copyWithCustomProcessor = useCallback((content: string) => {
    if (!customProcessor) {
    ...
    const description = processedContent.length > maxDescriptionLength 
  ...
  const copyContent = useCallback((contentOrElement: string | HTMLElement) => {
    switch (mode) {
      case 'element':
