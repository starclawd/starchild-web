...
import { liveTradingDomain } from 'utils/url'
 ...
 * 处理认证和错误提示等通用逻辑
 */
const liveTradingBaseQueryWithIntercept: BaseQueryFn<string | FetchArgs, unknown, FetchBaseQueryError> = async (
  args,
  api,
  extraOptions,
) => {
  ...
  // 发送请求
  const result = await baseQuery(liveTradingDomain['restfulDomain'])(
    {
      ...(args as FetchArgs),
...
export const liveTradingApi = createApi({
  baseQuery: liveTradingBaseQueryWithIntercept,
