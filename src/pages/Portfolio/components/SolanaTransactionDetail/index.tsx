...
import {
  ...
  SolanaWalletTransactionDetailDataType,
...
export default function SolanaTransactionDetail({
  ...
  const [solanaTransactionDetail, setSolanaTransactionDetail] = useState<SolanaWalletTransactionDetailDataType | null>(
    null,
  )
  ...
  const txInfo = useMemo(() => {
    ...
    const hasAggTokenSwap =
      detailData.activities &&
      detailData.activities.some((activity) => activity.activity_type === 'ACTIVITY_AGG_TOKEN_SWAP')

    if (hasAggTokenSwap && detailData.summaries && detailData.summaries.length > 0) {
      // 对于聚合Swap，我们应该直接从summaries获取源代币和目标代币信息
      const swapSummary = detailData.summaries.find(
        (summary) => summary.title && summary.title.activity_type === 'ACTIVITY_AGG_TOKEN_SWAP',
      )
    ...
    if (detailData.transfers && detailData.transfers.length > 0) {
      ...
      if (totalOutgoing > 0 && totalIncoming > 0) {
        ...
        // 检查代币类型
        if (
          firstTransfer.token_address &&
          firstTransfer.token_address !== 'So11111111111111111111111111111111111111111'
        ) {
          // 从metadata获取symbol
        ...
        list: [
          {
            ...
            value: (
              <HashWrapper>
                <Left>
                  <span>{hash}</span>
                  <DetailButton onClick={goHashPage(hashLink)}>
          ...
          },
          {
            ...
            value: (
              <TimeWrapper>
                {formatTimestamp(
                  solanaTransactionDetail.data.block_time
                    ? solanaTransactionDetail.data.block_time * 1000
                    : solanaTransactionDetail.data.time,
                )}
              </TimeWrapper>
            ),
          },
  ...
  if (!solanaTransactionDetail) {
    return (
      <TransactionDetailWrapper ref={scrollRef} className='scroll-style'>
        <Pending isFetching />
