...
const getTransactionTypeInfo = (data: EvmWalletOriginalHistoryDataType) => {
  ...
  if (data.method_label) {
    ...
    if (methodLabel === 'approve' || methodLabel === 'permit') {
      ...
      methodLabel === 'add liquidity' ||
      methodLabel === 'remove liquidity' ||
      methodLabel === 'zap in' ||
      methodLabel === 'zap out'
    ) {
      icon = 'liquidity'
  ...
  if (data.receipt_status === '0') {
    ...
    if (
      (!data.erc20_transfers || data.erc20_transfers.length === 0) &&
...
export default function TransactionItem({
  ...
  return (
    <TransactionItemWrapper key={originalResult.hash} onClick={handleClick}>
      <ItemLeft>
        <IconWrapper>
