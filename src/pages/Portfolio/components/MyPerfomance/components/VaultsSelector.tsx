...
const VaultsSelector = memo(() => {
  ...
  const { protocolVaults, communityVaults, isLoadingVaults } = useVaultsData()

  // 合并所有金库数据
  const allVaults = useMemo(() => {
    return [...protocolVaults, ...communityVaults]
  }, [protocolVaults, communityVaults])
  ...
  const vaultOptions: DataType[] = useMemo(() => {
    ...
    return allVaults.map((vault) => ({
      value: vault.id,
      text: vault.name,
      clickCallback: () => setChartVaultId(vault.id),
    }))
  ...
  useEffect(() => {
    if (!isLoadingVaults && allVaults.length > 0 && !chartVaultId) {
      setChartVaultId(allVaults[0].id)
    }
  ...
  const getSelectedVaultName = () => {
    ...
    const selectedVault = allVaults.find((vault) => vault.id === chartVaultId)
    return selectedVault?.name || allVaults[0]?.name || t`No vaults`
  }
