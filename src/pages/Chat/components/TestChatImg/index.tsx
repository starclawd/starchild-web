...
import { useGetChartImg, useGetOpenAiData } from 'store/chat/hooks'
...
export default function TestChatImg({ data }: { data: TempAiContentDataType }) {
   ...
   */
  const addTextToImageWithOptions = useCallback(
    async (text: string, imageUrl: string, parameter: any): Promise<string> => {
      try {
  ...
  const getChartImg = useCallback(
    async (testConfig: any) => {
      ...
      if (testConfig.trend_price && testConfig.trend_price.length > 0) {
        const filterTrendPrice = testConfig.trend_price.filter((item: number[]) => {
          ...
          if (testConfig.detail_description.includes('bearish')) {
            ...
            testConfig.detail_description.includes('bullish') ||
            testConfig.detail_description.includes('neutral')
          ) {
    ...
    try {
      ...
      if (resultList.length > 0) {
        ...
        const processedImages = await Promise.all(
          resultList.map(async (item: any, index: number) => {
            const detailDescription = item.detailDescription
            const parameter = item.parameter
            const imgUrl = item.url
            try {
              // 在图片上添加用户输入的文本作为标题
              const imageWithText = await addTextToImageWithOptions(detailDescription, imgUrl, parameter)
  ...
  return (
    <IconWrapper $borderRadius={16} $borderColor={theme.black600} onClick={testChatImg}>
      ...
      {isShowModal && (
        <ImgModal imgList={imgList} isShowModal={isShowModal} toggleTestChatImgModal={() => setIsShowModal(false)} />
      )}
    </IconWrapper>
  )
}
