...
export function getInsightTitle(data: InsightsDataType, isInsightTitle: boolean) {
  ...
  if (alertType === ALERT_TYPE.PRICE_ALERT) {
    return (
      <TitleWrapper $isInsightTitle={isInsightTitle} $isLong={isLong}>
        {isInsightTitle ? (
          <Trans>
            <span className='symbol'>{symbol}</span> <span className='change'>{change}</span> within 15m
          </Trans>
        ) : (
    ...
    return (
      <TitleWrapper $isInsightTitle={isInsightTitle} $isLong={isLong}>
        {isInsightTitle ? (
          <Trans>
            <span className='symbol'>{symbol}</span> <span className='change'>{change24h}</span> within 24H
          </Trans>
        ) : (
    ...
    return (
      <TitleWrapper $isInsightTitle={isInsightTitle} $isLong={isLong}>
        {isInsightTitle ? (
          <Trans>
            <span className='symbol'>{symbol}</span> <span className='change'>{formatValue}</span>{' '}
            <span>{sideText}</span>
          </Trans>
        ) : (
          <Trans>
            <span className='change'>{formatValue}</span> <span>{sideText}</span>
          </Trans>
        )}
      </TitleWrapper>
    )
    ...
    return (
      <TitleWrapper $isInsightTitle={isInsightTitle} $isLong={isLong}>
        {isInsightTitle ? (
          <Trans>
            <span className='symbol'>{symbol}</span> funding rate is <span className='change'>{fundingRate}%</span>
          </Trans>
        ) : (
          <Trans>
            <span className='change'>{fundingRate}%</span>
          </Trans>
        )}
      </TitleWrapper>
    )
    ...
    return (
      <TitleWrapper $isInsightTitle={isInsightTitle} $isLong={isLong}>
        {isInsightTitle ? <Trans>{postContent}</Trans> : <Trans>{postContent}</Trans>}
      </TitleWrapper>
    )
  }
...
export default function Tooltip({ markerData }: { markerData: MarkerPoint }) {
  ...
  const [isLong, insightTitle] = useMemo(() => {
    ...
    if (currentShowId) {
      const matchedData = markerData.originalList.find((data) => data.id.toString() === currentShowId)
  ...
  return (
    <TooltipWrapper $isLong={isLong} $isTop={isLong}>
      <span className='tooltip-shape'>
        <span className='tooltip-shape-dot'></span>
        <IconToolShape />
        <IconToolShape />
      </span>
      <span className='tooltip-text'>{insightTitle}</span>
    </TooltipWrapper>
  )
}
