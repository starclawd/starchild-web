...
const SingleMarker: React.FC<SingleMarkerProps> = ({
  ...
  useEffect(() => {
    ...
    const matchFound = markerData.originalList.some((data) => currentShowId === data.id.toString())
  ...
  // 查找时间点对应的数据值
  const findValueForTime = useCallback(
    (time: string | UTCTimestamp): number | null => {
      // 标准化时间格式以便比较
  ...
  return (
    <>
      <MarkerDot
        ...
        onClick={handleClick}
      >
        {(isHovered || isMatched) && <Tooltip markerData={markerData} />}
      </MarkerDot>
    </>
  )
}
...
const Markers: React.FC<MarkersProps> = ({
  ...
  const filterInsightsList = useMemo(() => {
    return insightsList.filter(
      (insight) =>
        !currentInsightToken ||
        (currentInsightToken && insight.marketId.toUpperCase() === currentInsightToken.toUpperCase()),
    )
    ...
    try {
      ...
      for (const insight of filterInsightsList) {
        ...
        if (closestDataPoint) {
          // 将这个时间点记录到映射表中
          const chartTime =
            typeof closestDataPoint.time === 'string'
