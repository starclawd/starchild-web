...
export const SYSTEM_PROMPT = JSON.stringify({
  ...
  output_format:
    'Return ONLY an array of request body JSON objects that would be sent to the chart-img.com API. If single symbol, return array with 1 object. If multiple symbols, return array with multiple objects.',
  api_info: {
  ...
  CRITICAL_STRUCTURE_RULES: {
    ...
    drawings_lines_only:
      "Use 'drawings' ONLY for graphical tools such as 'Trend Line', 'Horizontal Line', 'Vertical Line', 'Fib Retracement', 'Text', etc. NEVER include any indicators here.",
    enforced_array_check:
      'For EACH item mentioned by user, determine: is it an indicator (→ studies) or line/shape (→ drawings)? No exceptions.',
  ...
  parameter_value_rules: {
    override_handling:
      "When using 'overrides', use actual values. Example: { 'MACD.linewidth': 2 }, NOT parameter object.",
  ...
  detail_description_rules: {
    ...
    format:
      'Numbered points, using plain text (no markdown). Separate studies and drawings clearly. Each item must be a separate point.',
  ...
  trend_price_rules: {
    example: [
      {
        user_input_example:
          'If a small double bottom forms in the 37-38 zone with volume breakout above 40, bulls may test 43+; conversely if 37 breaks, short term may pull back to 35 to find buyers',
        trend_price_return: [
          [37, 40, 43],
          [37, 35],
        ],
