...
export default memo(function VolumeChart({
  ...
  const initial_value = strategyBacktestData?.result?.summary?.initial_capital || 0
  const { coingecko_id, base: symbol } = currentSymbolData
  const funding_trends = useMemo(
    () => strategyBacktestData?.result?.funding_trends || [],
    [strategyBacktestData?.result?.funding_trends],
  )
