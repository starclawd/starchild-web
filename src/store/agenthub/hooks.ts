...
import { 
  updateSignalScannerAgents, 
  updateSignalScannerList, 
  updateIsLoading 
} from './reducer'
...
export function useSignalScannerAgents(): [SignalScannerAgent[], (agents: SignalScannerAgent[]) => void] {
  ...
  const setSignalScannerAgents = useCallback((agents: SignalScannerAgent[]) => {
...
export function useSignalScannerList(): [
  ...
  const dispatch = useDispatch()
  const setSignalScannerList = useCallback((data: { data: SignalScannerAgent[], total: number, page: number, pageSize: number }) => {
...
export function useGetSignalScannerList() {
  ...
  return useCallback(async (params: SignalScannerListParams) => {
    try {
      setIsLoading(true)
