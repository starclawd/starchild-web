...
import {
  AllEvmWalletTokensDataType,
  AllSolanaWalletTokensDataType,
  NetWorthDataType,
...
export function useGetWalletHistory() {
  ...
  return useCallback(
    async ({ evmAddress, limit, chain }: { evmAddress: string; limit: number; chain: Chain }) => {
      try {
...
export function useWalletHistory(): [
  (WalletHistoryDataType | SolanaWalletHistoryDataType)[],
  (walletHistory: (WalletHistoryDataType | SolanaWalletHistoryDataType)[]) => void,
] {
  const walletHistory = useSelector((state: RootState) => state.portfolio.walletHistory)
  const dispatch = useDispatch()
  const setWalletHistory = useCallback(
...
export function useGetWalletNetWorth() {
  ...
  return useCallback(
    async ({ evmAddress, chains }: { evmAddress: string; chains: Chain[] }) => {
      try {
...
export function useNetWorthList(): [NetWorthDataType[], (netWorthList: NetWorthDataType[]) => void] {
  ...
  const dispatch = useDispatch()
  const setNetWorthList = useCallback(
      ...
      try {
        ...
        list.sort((a, b) => {
          if (a.chain === Chain.SOLANA) {
            const usdValueA = mul(
              (a as AllSolanaWalletTokensDataType).amount,
              (a as AllSolanaWalletTokensDataType).tokenDetail?.usdPrice,
            )
            const usdValueB = mul(
              (b as AllSolanaWalletTokensDataType).amount,
              (b as AllSolanaWalletTokensDataType).tokenDetail?.usdPrice,
            )
            return Number(usdValueB) - Number(usdValueA)
          } else {
            return (
              Number((b as AllEvmWalletTokensDataType).usd_value) - Number((a as AllEvmWalletTokensDataType).usd_value)
            )
          }
        })
        const filteredList = list.filter((item) => {
          if (item.chain !== Chain.SOLANA) {
            return (
              (item as AllEvmWalletTokensDataType).verified_contract &&
              !(item as AllEvmWalletTokensDataType).possible_spam
            )
          } else {
            return (
              !(item as AllSolanaWalletTokensDataType).possibleSpam &&
              !!(item as AllSolanaWalletTokensDataType).tokenDetail &&
              (item as AllSolanaWalletTokensDataType).logo
            )
          }
        })
...
export function useAllNetworkWalletTokens(): [
  (AllEvmWalletTokensDataType | AllSolanaWalletTokensDataType)[],
  (allNetworkWalletTokens: (AllEvmWalletTokensDataType | AllSolanaWalletTokensDataType)[]) => void,
] {
  const allNetworkWalletTokens = useSelector((state: RootState) => state.portfolio.allNetworkWalletTokens)
  const dispatch = useDispatch()
  const setAllNetworkWalletTokens = useCallback(
