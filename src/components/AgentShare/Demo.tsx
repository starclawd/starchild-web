...
import AgentShare, { useCopyImgAndText } from './index'
import { GENERATION_STATUS, AGENT_TYPE, AgentDetailDataType, AGENT_STATUS } from 'store/agentdetail/agentdetail'
...
const AgentShareDemo = () => {
  const [isCopyLoading, setIsCopyLoading] = useState(false)
  const shareDomRef = useRef<HTMLDivElement>(null)
  const copyImgAndText = useCopyImgAndText()

  // 模拟任务详情数据
  const mockAgentDetail: AgentDetailDataType = {
    task_id: 'demo-task-001',
    description: '这是一个用于演示的任务分享组件，展示了如何使用 AgentShare 组件来分享任务信息。',
    user_name: 'Demo User',
  ...
  return (
    <DemoContainer>
      <DemoSection>
        <h2>AgentShare 任务分享组件示例</h2>
        <p>
          AgentShare 组件用于生成任务分享的预览图，包含任务描述、用户信息、历史聊天记录和二维码等信息。
      ...
      <DemoSection>
        ...
        <DemoRow>
          ...
          <div className='share-preview'>
            ...
            <AgentShare agentDetailData={mockAgentDetail} ref={shareDomRef} shareUrl={shareUrl} />
          </div>
        </DemoRow>
...
<button onClick={handleShare}>分享图片</button>`}
      ...
      <DemoSection>
        ...
        >
          <h4 style={{ marginBottom: '15px', color: '#4ecdc4' }}>AgentDetailDataType</h4>
...
// 使用示例
const agentDetailData: AgentDetailDataType = {
  task_id: 'demo-task-001',
  description: '任务描述',
  user_name: 'Demo User',
  ...
  trigger_history: [
    {
      ...
      <DemoSection>
        ...
        >
          ...
          >
            <div style={{ fontFamily: 'monospace', color: '#ff6b6b' }}>agentDetailData</div>
            <div style={{ fontFamily: 'monospace', color: '#4ecdc4' }}>AgentDetailDataType</div>
            <div>任务详情数据</div>
          </div>
  ...
  agentDetailData: AgentDetailDataType  // 必填：任务详情数据
