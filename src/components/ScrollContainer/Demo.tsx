...
const ScrollContainerDemo = () => {
  ...
  const generateItems = (count: number) => Array.from({ length: count }, (_, i) => `列表项 ${i + 1} - 这是一个示例内容`)

  const generateHorizontalItems = (count: number) => Array.from({ length: count }, (_, i) => `卡片 ${i + 1}`)
            ...
            <ScrollContainer style={{ height: '200px', width: '300px' }}>
              <ExampleList>
                {generateItems(20).map((item, index) => (
                  <ExampleItem key={index}>{item}</ExampleItem>
                ))}
              </ExampleList>
            </ScrollContainer>
          </div>
            ...
            <ScrollContainer style={{ width: '400px', maxWidth: '100%' }}>
              <HorizontalList>
                {generateHorizontalItems(10).map((item, index) => (
                  <HorizontalItem key={index}>{item}</HorizontalItem>
                ))}
              </HorizontalList>
            </ScrollContainer>
          </div>
          ...
          <div className='demo-area' style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <input
                ...
                style={{ width: '200px' }}
              />
              <span style={{ color: '#1890ff', fontSize: '14px', fontFamily: 'monospace' }}>{customHeight}px</span>
            </div>
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          <div style={{ background: 'rgba(255, 255, 255, 0.05)', padding: '20px', borderRadius: '8px' }}>
            <ul style={{ margin: 0, paddingLeft: '20px', lineHeight: '1.6' }}>
              <li>
                <strong>智能滚动检测</strong>：自动检测是否需要显示滚动条
              </li>
              <li>
                <strong>自定义滚动条样式</strong>：在桌面端显示美观的细滚动条
              </li>
              <li>
                <strong>移动端优化</strong>：在移动设备上隐藏滚动条，使用原生滚动
              </li>
              <li>
                <strong>悬停效果</strong>：桌面端鼠标悬停时滚动条变为可见状态
              </li>
              <li>
                <strong>自适应间距</strong>：有垂直滚动条时自动调整右边距
              </li>
            </ul>
          </div>
        </div>
