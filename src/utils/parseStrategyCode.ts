...
function extractSignalDefinitions(code: string): {
  ...
  if (!code || typeof code !== 'string') return signals
...
function inferStrategyType(code: string): string {
  if (!code || typeof code !== 'string') return 'Trading Strategy'
...
function extractConfig(code: string): StrategyConfig {
  ...
  try {
    ...
    console.error('[extractConfig] Failed to extract config:', error)
    return defaultConfig
  }
}
...
function extractDataSources(code: string): DataSourceNode[] {
  ...
  if (!code || typeof code !== 'string') return sources
  ...
  try {
    ...
    if (code.includes('api.taapi.io') || code.includes('taapi.io/bulk')) {
      ...
      sources.push({
        ...
        type: 'datasource',
        api: 'TAAPI',
        fields: fields.length ? fields : ['Indicators'],
      })
    }
    ...
    console.error('[extractDataSources] Failed to extract data sources:', error)
    return [{ id: 'ds-default', type: 'datasource', api: 'Exchange API', fields: ['Price', 'Volume'] }]
  }
}
...
function extractIndicators(code: string): IndicatorNode[] {
  ...
  if (!code || typeof code !== 'string') return indicators
  ...
  try {
    ...
    if (hasRsiLogic) {
      ...
      // 提取 RSI 阈值 - 支持多种格式
      const oversoldLevel =
        code.match(/rsi_oversold["\s:]*(\d+)/i)?.[1] ||
    ...
    console.error('[extractIndicators] Failed to extract indicators:', error)
    return indicators
  }
}
...
function extractConditionsFromDocstring(docstring: string): { entry: string[]; exit: string[] } {
  ...
  if (!docstring || typeof docstring !== 'string') return { entry, exit }
...
function extractEntryConditions(code: string, docstring: string): ConditionNode[] {
  ...
  if (!code || typeof code !== 'string') return conditions
  ...
  try {
    ...
    console.error('[extractEntryConditions] Failed to extract entry conditions:', error)
    return conditions
  }
}
...
function extractExitConditions(code: string, config: StrategyConfig): ConditionNode[] {
  ...
  try {
    ...
    console.error('[extractExitConditions] Failed to extract exit conditions:', error)
    return conditions
  }
}
...
function extractCandlePattern(code: string, docstring: string): CandlePatternInfo | undefined {
  ...
  const safeDocstring = docstring && typeof docstring === 'string' ? docstring : ''
  ...
  try {
    ...
    const consecutiveGreenMatch =
      code.match(/consecutive_greens?\s*>=?\s*(\d+)/i) ||
      code.match(/(\d+)\s*consecutive\s*green/i) ||
      safeDocstring.match(/(\d+)\s*consecutive\s*green/i) ||
    ...
    const consecutiveRedMatch =
      ...
      safeDocstring.match(/(\d+)\s*consecutive\s*red/i)
    ...
    if (code.includes('candle_colors') || code.includes('candle["close"] > candle["open"]')) {
      ...
      const entryMatch = safeDocstring.match(/Entry[:\s]*(.*?)(?=Exit|$)/i)?.[1]?.trim()
      const exitMatch = safeDocstring.match(/Exit[:\s]*(.*?)(?=Timeframe|Data|$)/i)?.[1]?.trim()
    ...
    console.error('[extractCandlePattern] Failed to extract candle pattern:', error)
    return undefined
  }
}
...
function extractStateManagement(code: string): StateManagementInfo | undefined {
  ...
  try {
    ...
    console.error('[extractStateManagement] Failed to extract state management:', error)
    return undefined
  }
}
...
function extractCalculations(code: string, docstring: string): string[] {
  ...
  if (!code || typeof code !== 'string') return calculations
  const safeDocstring = docstring && typeof docstring === 'string' ? docstring : ''

  try {
    // 从 docstring 提取
    if (safeDocstring.includes('candle')) {
      const candleMatch = safeDocstring.match(/last\s*(\d+).*?candle/i)
      if (candleMatch) {
    ...
    console.error('[extractCalculations] Failed to extract calculations:', error)
    return calculations
  }
}
...
function extractAnalyzeSteps(code: string, docstring: string): AnalyzeStep[] {
  ...
  try {
    ...
    console.error('[extractAnalyzeSteps] Failed to extract analyze steps:', error)
    return [{ id: 'step-analyze', label: 'Analyze Market', description: 'Process market data and indicators' }]
  }
}
...
function extractDecisionLogic(
  ...
  const defaultHasPosition: DecisionBranch[] = [
    { condition: 'Exit condition met', action: 'CLOSE', description: 'Close existing position' },
  ]
  const defaultNoPosition: DecisionBranch[] = [
    { condition: 'Entry signal triggered', action: 'OPEN', description: 'Open new position' },
  ]

  if (!code || typeof code !== 'string') {
    return { hasPosition: defaultHasPosition, noPosition: defaultNoPosition }
  }
  ...
  try {
    ...
    console.error('[extractDecisionLogic] Failed to extract decision logic:', error)
    return { hasPosition: defaultHasPosition, noPosition: defaultNoPosition }
  }
}
